...
RUN /home1/irteam/apps/mysql/bin/mysqld --initialize && \
echo -e "[mysqld]" >> /home1/irteam/apps/mysql/my.cnf && \ 
...
sed -i '4720i\<description>MySQL Test App</description> <resource-ref> <description>DB Connection</description> <res-ref-name>jdbc/tikim</res-ref-name> <res-type>javax.sql.DataSource</res-type> <res-auth>Container</res-auth> </resource-ref>' /home1/irteam/apps/worker1/conf/web.xml && \
sed -i '4720i\<description>MySQL Test App</description> <resource-ref> <description>DB Connection</description> <res-ref-name>jdbc/tikim</res-ref-name> <res-type>javax.sql.DataSource</res-type> <res-auth>Container</res-auth> </resource-ref>' /home1/irteam/apps/worker2/conf/web.xml && \
...
sed -i '41i\<Resource name="jdbc/tikim" auth="Container" type="javax.sql.DataSource" maxTotal="100" maxIdle="30" maxWaitMillis="10000" username="root" password="root1234" driverClassName="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:13306/testdb"/>' /home1/irteam/apps/worker1/conf/context.xml && \
sed -i '47i\<Resource name="jdbc/tikim" auth="Container" type="javax.sql.DataSource" maxTotal="100" maxIdle="30" maxWaitMillis="10000" username="root" password="root1234" driverClassName="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:13306/testdb"/>' /home1/irteam/apps/worker2/conf/context.xml && \
echo -e "<%@ taglib prefix=\"c\" uri=\"http://java.sun.com/jsp/jstl/core\" %> <%@ taglib prefix=\"sql\" uri=\"http://java.sun.com/jsp/jstl/sql\" %> <\!DOCTYPE html> <html> <head> <meta charset=\"UTF-8\"> <title>OJT DB select</title> </head> <body> <p> JSTL DB Select </p> <sql:query var=\"rs\" dataSource=\"jdbc/tikim\"> select sno, prof, dept, cno, grade from course; </sql:query> <table border=\"1\"> <tr> <th>sno</th><th>prof</th><th>dept</th><th>cno</th><th>grade</th> <c:forEach var=\"s\" items=\"\${rs.rows}\"> <tr> <td>\${s.sno}</td> <td>\${s.prof}</td> <td>\${s.dept}</td> <td>\${s.cno}</td> <td>\${s.grade}</td> </tr> </c:forEach> </tr> </table> </body> </html>" >> /home1/irteam/apps/worker1/webapps/ROOT/test.jsp && \
...
/home1/irteam/apps/worker1/bin/startup.sh && \
/home1/irteam/apps/worker2/bin/startup.sh
